...
从结果看，混合检索在实际执行检索的样本上取得 93.30% 的 Recall@5；当前主要改进空间是长尾延迟与证据不足场景下的重试成本。完整的 LLM 调用统计、分层指标和逐 case 重试诊断见 [100 条测试数据](docs/100条测试数据.md)，评测命令见 [开发命令](.agent-harness/03_DEV_COMMANDS.md)。
