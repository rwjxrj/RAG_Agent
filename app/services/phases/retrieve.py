...
from app.services.schemas import RetrievePhaseOutput, RetrieveResult
...
async def execute_retrieve(
    ...
    *,
    ...
    ctx.retrieve_output = RetrievePhaseOutput(
        retry_strategy_applied=retry_strategy_applied,
        active_required_evidence=list(retrieval_plan.active_required_evidence or []),
        active_hard_requirements=list(retrieval_plan.active_hard_requirements or []),
        active_soft_requirements=list(retrieval_plan.active_soft_requirements or []),
        active_hypothesis_name=retrieval_plan.active_hypothesis_name,
        active_answer_shape=retrieval_plan.answer_shape,
        active_evidence_families=list(retrieval_plan.evidence_families or []),
        evidence_eval_result=ctx.retrieve_output.evidence_eval_result,
        hypothesis_history=list(ctx.retrieve_output.hypothesis_history),
    )
    ...
    if evidence:
        ...
        if eval_task is not None:
            ...
            if not isinstance(eval_result, Exception):
                ctx.retrieve_output.evidence_eval_result = eval_result
            else:
                _pipeline_log(
                    "retrieve",
    ...
    ctx.retrieve_output.hypothesis_history = history
