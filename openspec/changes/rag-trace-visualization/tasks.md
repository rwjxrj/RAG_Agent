...
- [x] 2.1 梳理当前 streaming conversation SSE 事件格式，确定可选 `trace` 事件的兼容写法。
- [x] 2.2 在流式入口输出节点开始、完成、跳过或回退的增量 trace 事件。
- [x] 2.3 确保不支持 trace 事件的客户端仍能按现有方式接收最终答案。

## 3. 前端可视化

- [x] 3.1 在问答界面新增轻量执行时间线，显示当前节点、已完成节点、跳过节点和失败回退状态。
- [x] 3.2 展示 Agentic Router 的 `selected_tool`、`decision_reason` 和置信度或回退状态。
- [x] 3.3 展示 `tool_result` 摘要，包括最终 decision、引用数量、追问数量和总耗时。
- [x] 3.4 对未知未来节点使用通用节点渲染，保证内部流程重构时前端仍兼容。
